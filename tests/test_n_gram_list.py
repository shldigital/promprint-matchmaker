...
def test_multi_n_gram_frequency():
    ...
    expected_data_series = pd.Series(data=n_gram_output)
    assert_series_equal(n_gram_series, expected_data_series)


expected_n_gram_series_sorted = {
    "the quick brown fox jumps over the lazy dog": 1,
    "the quick brown fox jumps over the lazy": 2,
    "quick brown fox jumps over the lazy dog": 1,
    "the quick brown fox jumps over the": 3,
...
def test_sort_n_grams_by_degree():
    n_gram_series = pd.Series(n_gram_output)
    n_gram_series_sorted = sort_n_grams_by_degree(n_gram_series)
    sorted_data = pd.Series(expected_n_gram_series_sorted, name=0)
    assert_series_equal(n_gram_series_sorted, sorted_data)
